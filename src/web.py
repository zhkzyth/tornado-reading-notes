...
class RequestHandler(object):
    ...
    _remove_control_chars_regex = re.compile(r"[\x00-\x08\x0e-\x1f]") # what is it?
    ...
    # 一次性把write_buffer的内容全部发送出socket对应的客户端，但这个时候连接是还没断开的
    ...
    def _execute(self, transforms, *args, **kwargs):
        ...
        try:
            ...
            if self.request.method not in ("GET", "HEAD", "OPTIONS") and \
                ...
                self.check_xsrf_cookie() # TODO 有空看下xsrf的验证是怎么做的，需求是什么？
            self._when_complete(self.prepare(), self._execute_method)
        except Exception as e:
            self._handle_request_exception(e)

    # 做了一层胶水，负责粘合多个具有先后顺序执行的函数，使他们可以拿到前面函数执行的结果
...
class Application(object):
    ...
    which will be used as keyword arguments(注意哦，是keyword参数，不是基于position的)
    to the handler's constructor and `~RequestHandler.initialize` method.  This pattern
    ...
    (static file handler也是基于常规的handler来实现的，只是为了应对常规的网站业务，加入了这些“框架糖”)
    ...
    def __init__(self, handlers=None, default_host="", transforms=None,
        ...
        if self.settings.get('autoreload') and not wsgi:
            ...
            autoreload.start()  # TODO 有空看看人家的autoreload是怎么实现的
    ...
    def add_handlers(self, host_pattern, host_handlers):
        ...
        for spec in host_handlers:
            ...
            # named_handlers 为啥要搞这个，感觉按照pattern优先原则，
            # 这里的handlers就没必要去时候重复的检测了
            if spec.name:
...
class URLSpec(object):
    ...
    def __init__(self, pattern, handler, kwargs=None, name=None):
        ...
        * ``name`` (optional): A name for this handler.  Used by
          `Application.reverse_url`. (TODO 这块的作用是啥)
        """
        if not pattern.endswith('$'):
            pattern += '$'
        self.regex = re.compile(pattern)
        # 这里处理的很干脆，要么全是key-value，要么全是position参数
    ...
    def reverse(self, *args):
        ...
        return self._path % tuple(converted_args) # 感觉这里是利用了"/"为参数切分做准备
