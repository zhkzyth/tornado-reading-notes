        ...
        while True:
            try:
                ...
                if e.args[0] == errno.ECONNABORTED:
                    continue
                raise  # 当标志可读，事实上却不可读的时候，就需要及早抛出错误了....不过什么情况下会导致这种情况呢？
            callback(connection, address)
    io_loop.add_handler(sock.fileno(), accept_handler, IOLoop.READ) #基本上所有的事件加入和事件触发，都是放在ioloop循环里面，又ioloop主循环负责调度
