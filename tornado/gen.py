...
could be written with ``gen`` as::
    ...
    class GenAsyncHandler(RequestHandler):
        ...
        def get(self):
            http_client = AsyncHTTPClient()
            response = yield http_client.fetch("http://example.com")   # 找到了，这里是需要我们自己来手动添加yield的，自己调度
            do_something_with_response(response)
            self.render("template.html")
...
# coroutine 包装了一个iterator对象，由调用者自己给出一个yield 关键字完成
...
# Runner启动之后，第一个YieldPoint永远都是Null类型的
# 后来再根据具体的类型做转换
...
# Callback和Wait的流程，可以结合调用API的例子和coroutine
...
# 利用Wait和Callback，我们就可以把计算和结果分开，不一定要写在一个逻辑里面
# 这样的设计，感觉像是消息队列，解耦合
# 内部实现是用一个hash表，任何时候获取到合适的数据
...
# 原来所谓的异步http，其真实的实现是放了一个新的线程来做。因为利用了python本身提供的concurrent库，
# 采用concurrent.ThreadingPool之后，提交的func（即任务）会立即返回一个future对象。而神奇的地方就在这个future对象了。
# future对象提供一系列的上层接口，告诉当前任务是否完成。然后tornado的作者很聪明的把future对象加入到基于linux系统的事件循环，
# 比如epoll或者select这些实现中，通过在事件循环里面，不断轮训当前队列的任务情况（也就是我们的future对象了），来采取下一步的措施，
# 比如重新加入轮训之类的？?

# epoll、事件循环、future的关系
# 其实，epoll只是tornado用来监控socket读写情况的工具
...
class Runner(object):
    ...
    Maintains information about pending callbacks and their results. # 一个中间存储器?

    ``final_callback`` is run after the generator exits.
    """
    def __init__(self, gen, final_callback):
        self.gen = gen  # 这里的gen代表什么呢?
            ...
            while True:
                ...
                try:
                    if self.exc_info is not None:
                        ...
                        yielded = self.gen.send(next) # send....也就是说整个系统是建构在yiled的基础上的，协程
                        # http://www.tornadoweb.org/en/stable/gen.html 查看下这个例子
                        # 假设我们是在yiled http_client.fetch()进行的，那么next下返回的结果其实就是一个Future对象
