...
class HTTPServer(TCPServer):
    ...
    """
    # the callback we pass here is from the app layer.
    # so when we get a connection from client, we just parse the body, and
    # get the parsed data back to our app layer.
    ...
    # 当TCP连接建立成功的时候，我们就需要iostream模块帮我们处理字节数据的处理，为上层http connection提供HTTP方面的语义。
    # 其实也就是实现一个更加高级的语义了
...
class HTTPConnection(object):
    ...
    def __init__(self, stream, address, request_callback, no_keep_alive=False,
        ...
        self._header_callback = stack_context.wrap(self._on_headers) # 什么是stack_context？
    ...
    # 当iostream层读完整个header字节的时候，就回调这里的_on_header内部函数进行处理，相当于在http connection层对底层iostream加了
    # 一个hook
